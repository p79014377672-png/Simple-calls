...
function setupControlButtons() {
    ...
    if (shareButton) {
        shareButton.onclick = shareRoomLink;
        shareButton._listenerAttached = true;
    }
}
...
// Функция для временных уведомлений (оставляем только уникальные)
function showTemporaryNotification(message, type) {
    ...
    if (duplicateMessages.includes(message)) {
        return; // Не показываем дубликаты
    }
...
// Мобильные уведомления (ПЕРЕМЕЩЕНЫ ВНИЗ ЭКРАНА)
...
function shareRoomLink() {
    ...
    if (navigator.share) {
        // Для мобильных устройств с поддержкой Web Share API
        navigator.share({
            title: 'Видео звонки визови.рф',
            text: 'Присоединяйтесь к видео звонку',
            url: roomLink
        }).then(() => {
            showMobileAlert('Ссылка скопирована');
        }).catch(err => {
            console.log('Ошибка при использовании Web Share API:', err);
            copyToClipboard(roomLink);
        });
    } else {
        // Для десктопов и старых браузеров
        copyToClipboard(roomLink);
    }
}
...
function copyToClipboard(text) {
    ...
    showMobileAlert('Ссылка скопирована в буфер');
}
...
document.addEventListener('DOMContentLoaded', function() {
    init();
    // Назначаем обработчики кнопок после полной загрузки DOM
